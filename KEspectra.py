...
def normalize_timeavg_field(field):
    """Normalize the field and get time average by subtracting the mean and dividing by the standard deviation."""
...
def plot_ke_spectrum(k, E, savepath):
    ...
    k_ref = np.logspace(np.log10(min(k)), np.log10(max(k)), 100)
    E_ref_53 = k_ref**(-5/3) * E[10] / k[10]**(-5/3)
    E_ref_3 = k_ref**(-3) * E[10] / k[10]**(-3)
